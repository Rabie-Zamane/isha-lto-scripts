...
def verify_virtual_tape(config, tape_id):
    ...
    if tape_id:
        tape_xml_path = ltoUtil.get_tape_pending_dir(config)+'/'+tape_id+'/'+tape_id+'.xml'
    else:
    ...
    for tar_elem in tar_elems:
        ...
        tarfile_path = ltoUtil.get_tape_pending_dir(config)+'/'+tape_id+'/'+tarfile_name
        ...
        if not stored_tar_md5 == ltoUtil.get_md5_hash(tarfile_path):
...
def setup_tape_drive(config):
    ...
    while True:
        p = subprocess.Popen('mt -f '+tape+' status', shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def write_tape(config, tape_id):
    ...
    tape_xml_path = ltoUtil.get_tape_pending_dir(config)+'/'+tape_id
    ...
    p = subprocess.Popen('tar -c -b '+str(blocking_factor)+' --format='+ltoUtil.get_tar_format(config)+' -C '+tape_xml_path+' -f '+tape_xml_tarfile_path+' '+tape_xml_file, shell=True)
    sts = os.waitpid(p.pid, 0)
    #Next use dd to pad the index tar file with null bytes so that it becomes the size given by tape_index_size
    index_tar_size = ltoUtil.get_filesize(tape_xml_tarfile_path)
    ...
    for index, tar in enumerate(tar_elems): 
        ...
        tar_path = ltoUtil.get_tape_pending_dir(config)+'/'+tape_id+'/'+tar_id+'.tar'
