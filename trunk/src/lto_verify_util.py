...
def verify_tape(config, tape_id):
    ...
    written_dir = lto_util.get_tape_written_dir(config)+'/'+tape_id
    temp_dir = lto_util.get_temp_dir(config)
    original_dir = temp_dir+'/verify-tape/'+tape_id+'/original'
    archived_dir = temp_dir+'/verify-tape/'+tape_id+'/archived'
    lto_util.create_dir(original_dir)
    lto_util.create_dir(archived_dir)
    ...
    if not os.path.exists(original_index):
        ...
        print lto_util.get_script_name()+' script terminated.'
        sys.exit(2)
    if not lto_util.compare(original_index, archived_index):
    ...
    while True:
        ...
        lto_util.create_dir(tapedump_dir)
        lto_util.create_dir(diskdump_dir)
        session_id = ""
        device_code = ""
        #Extract tarfile members from tape
        print 'Extracting #'+str(position)+' tar archive from tape'
        ...
        if int(t.getAttribute('position')) == position:
            ...
            for file in os.listdir(diskdump_dir):
                ...
                md5 = lto_util.compare(fp1, fp2)
                if not md5:
                    print 'Verification Error: The file '+file+' failed the byte comparison with the local copy.'
                    print lto_util.get_script_name()+' script terminated.'
                    sys.exit(2)
                else:
                        ...
                        for c in children:
                            if c.nodeType == c.ELEMENT_NODE and c.getAttribute('filename') == file:
                                if md5 == c.getAttribute('md5'):
                                    ...
                                    print 'Verification Error: The file '+file+' failed the md5 verification against the index file.'
                                    print lto_util.get_script_name()+' script terminated.'
                                    sys.exit(2)
            ...
            for m in os.listdir(tapedump_dir):
                if not m in tar_members:
                    print 'Verification Error: The file '+m+' was found on the tape, but not on the corresponding tape archive '+tarfile_path 
                    print lto_util.get_script_name()+' script terminated.'
                    sys.exit(2)
